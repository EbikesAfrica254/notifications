...
@DisplayName("OrganizationPreferenceSpecifications")
class OrganizationPreferenceSpecificationsTest extends AbstractRepositoryTest {

  @Autowired private OrganizationPreferenceRepository organizationPreferenceRepository;

  private OrganizationChannelPreference emailTransactionalEnabled;
  private OrganizationChannelPreference smsTransactionalEnabled;
  private OrganizationChannelPreference emailMarketingDisabled;
    ...
    emailTransactionalEnabled =
        organizationPreferenceRepository.save(
            OrganizationPreferenceFixtures.enabled(
                TEST_ORGANIZATION_ID, ChannelType.EMAIL, NotificationCategory.TRANSACTIONAL));
    smsTransactionalEnabled =
        organizationPreferenceRepository.save(
            OrganizationPreferenceFixtures.enabled(
                TEST_ORGANIZATION_ID, ChannelType.SMS, NotificationCategory.TRANSACTIONAL));
  ...
  void noFilterReturnsAll() {
    List<OrganizationChannelPreference> results =
        organizationPreferenceRepository.findAll(
            OrganizationPreferenceSpecifications.buildSpecification(new ChannelPreferenceFilter()));
  ...
  void filterByEnabledTrue() {
    ...
    assertThat(results).hasSize(2).allMatch(OrganizationChannelPreference::getEnabled);
  }
